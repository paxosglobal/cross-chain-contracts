...
describe('OFTWrapperUpgradeable Send Test', function () {
  ...
  before(async function () {
    ...
    MyOFT = await ethers.getContractFactory('OFTWrapperUpgradeable');
  ...
  beforeEach(async function () {
    ...
    // Get token decimals
    const tokenDecimalsA = await tokenFixture.decimals();
    const tokenDecimalsB = await tokenFixtureB.decimals();

    // Deploy OFT A via proxy
    myOFTA = await upgrades.deployProxy(
      MyOFT.connect(ownerA),
      [ownerA.address, ownerA.address],
      {
        initializer: 'initialize',
        kind: 'uups',
        constructorArgs: [tokenDecimalsA, mockEndpointA.address, tokenFixture.address],
        unsafeAllow: ['constructor', 'state-variable-immutable', 'missing-initializer-call', 'missing-initializer']
      }
    );

    // Deploy OFT B via proxy
    myOFTB = await upgrades.deployProxy(
      MyOFT.connect(ownerB),
      [ownerB.address, ownerB.address],
      {
        initializer: 'initialize',
        kind: 'uups',
        constructorArgs: [tokenDecimalsB, mockEndpointB.address, tokenFixtureB.address],
        unsafeAllow: ['constructor', 'state-variable-immutable', 'missing-initializer-call', 'missing-initializer']
      }
    );

    // Set up rate limits after deployment
    const rateLimitConfigA = { dstEid: 2, limit: ethers.utils.parseEther('1000'), window: 100 }
    const rateLimitConfigB = { dstEid: 1, limit: ethers.utils.parseEther('1000'), window: 100 }
    const inboundRateLimitConfigA = { dstEid: 2, limit: ethers.utils.parseEther('1000'), window: 100 }
    const inboundRateLimitConfigB = { dstEid: 1, limit: ethers.utils.parseEther('1000'), window: 100 }

    await myOFTA.connect(ownerA).setOutboundRateLimits([rateLimitConfigA]);
    await myOFTA.connect(ownerA).setInboundRateLimits([inboundRateLimitConfigA]);
    await myOFTB.connect(ownerB).setOutboundRateLimits([rateLimitConfigB]);
    await myOFTB.connect(ownerB).setInboundRateLimits([inboundRateLimitConfigB]);
